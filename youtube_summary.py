...
CHANNEL_IDS_STR = os.environ.get("CHANNEL_ID") # 시크릿에서 콤마로 구분된 문자열을 받음
SPREADSHEET_NAME = "유튜브 요약"
...
def connect_google_sheet():
    ...
    try:
        ...
        try:
            ...
            print(f"📄 '{SPREADSHEET_NAME}' 시트를 새로 생성합니다.")
            new_sheet = client.create(SPREADSHEET_NAME)
    ...
    try:
        ...
        while True:
            ...
            for item in pl_res["items"]:
                ...
                # [★여기만 수정함] 2개가 모이면 즉시 종료 (반복문 탈출)
                if len(videos) >= 2:
                    break
            
            # 위에서 break로 나왔으면 여기서도 탈출
            if len(videos) >= 2:
                break
